...
//! real `MlsGroup` instances: Bob joins through `welcome.mls`, decrypts
//! `application-private.mls` matching `application-frame.cbor`, Alice verifies
//! own-private echo outcome, Bob resolves the metadata AppData commit via
//! `resolve_app_data_commit`, and Bob rejoins via `rejoin-welcome.mls`.
...
#[test]
fn native_crypto_wire_v09_consumer_join_decrypt_and_outcomes() {
    ...
    // Verify all 23 v09 artifacts are present and well-formed
    ...
    // 1. Live OpenMLS 0.9 consumer flow with Welcome join, decrypt, AppData, and outcomes
