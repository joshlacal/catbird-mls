...
async fn test_commit_messages_advance_epoch() {
    ...
    bob.orchestrator
        .swap_members(&group_id, &[], &[carol_did])
        .await
...
async fn test_add_members_server_rejection_cleanup() {
    ...
    let result = alice
        .orchestrator
        .swap_members(group_id, &[], &[bob.did.clone()])
        .await;
    assert!(
        result.is_err(),
...
async fn test_sync_rejoin_uses_stable_conversation_id_when_group_id_differs() {
    ...
    alice
        .orchestrator
        .swap_members(&conversation_id, &[], std::slice::from_ref(&bob_did))
        .await
...
async fn test_force_rejoin_cooldown_suppresses_immediate_retry() {
    ...
    let first = alice
        .orchestrator
        .force_rejoin(&convo.conversation_id)
        .await;
    assert!(
        alice
            .orchestrator
