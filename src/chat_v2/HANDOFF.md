...
**This charter is now discharged — see below.**

**The 11 signing domains are pinned; the charter above is closed.** mls-ds
seal `4650d12a` adds `server/tests/fixtures/mls_chat_signing_domain_vectors.json`,
generated by `server/tests/chat_protocol_signing_domain_vectors.rs` through the
server's own `decode_canonical_signed_mutation` /
`decode_and_verify_signed_mutation` — the entry points the live handlers call —
so every case is one the server actually accepted, key-id binding included.
catbird-mls seal `3645eb3b` vendors it verbatim and consumes it in
`transcript/domain_vector_tests.rs`, which rebuilds each transcript from the
wire body alone and reproduces the server's exact bytes for all eleven,
`CATBIRD-CHAT-MESSAGE` included. All 25 domains are now server-pinned, and the
accounting is derived on both sides rather than transcribed. Two things worth
carrying forward: the vectors went into a **sibling** fixture rather than into
`mls_chat_contract_vectors.json`, because that file's bytes are hashed into the
frozen `chat-application-v1` provenance and editing it desyncs that record
silently; and the eleven bodies are the first place in either repo where every
non-control signed body is written out in full, so they are the reference shape
for anyone constructing one.
...
| `wunxntmp` | `3645eb3b` | Pin the eleven remaining signing domains (mls-ds `4650d12a`) |
